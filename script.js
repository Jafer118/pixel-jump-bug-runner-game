...
function update() {
  ...
  if (state.phase === 'run') {
    ...
    if (state.distance >= state.nextBossDistance) {
      triggerBossIntro();
    } else if (state.distance >= state.nextFlightDistance) {
      triggerFlight();
    }
    ...
    if (state.distance >= state.nextFlightDistance + FLIGHT_DURATION_METERS) {
      ...
      if (state.nextFlightDistance <= state.nextBossDistance) {
        state.nextFlightDistance = state.nextBossDistance + FLIGHT_INTERVAL_METERS;
      }
