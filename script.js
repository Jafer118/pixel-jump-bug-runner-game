...
let state = {
  ...
  // Boss / afstand
  phase: 'run', // 'run' | 'boss_intro' | 'boss_fight' | 'boss_defeat'
  distance: 0,
...
function shoot() {
  ...
  state.shootCooldown = SHOOT_COOLDOWN_FRAMES;
  state.muzzleFlash = 6;

  const gunX = player.x + player.w + 6;
...
function update() {
  ...
  for (let i = obstacles.length - 1; i >= 0; i--) {
    const o = obstacles[i];
    o.x -= state.speed * o.type.speedMult;
...
function drawBackground() {
  ...
  sky.addColorStop(0, bossPhase ? '#220f1d' : '#111827');
  sky.addColorStop(1, bossPhase ? '#411d2a' : '#293247');
  ctx.fillStyle = sky;
...
function drawMuzzleAndTrail() {
  const gunTipX = player.x + 44 + 29;
  const gunTipY = player.y + 26 + (player.grounded ? Math.sin(player.runFrame * 0.5) * 2 : 0);
